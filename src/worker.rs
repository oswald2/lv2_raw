...
pub static LV2_WORKER_URI: &'static [u8] = b"http://lv2plug.in/ns/ext/worker\0";
pub static LV2_WORKER_PREFIX: &'static [u8] = b"http://lv2plug.in/ns/ext/worker#\0";

pub static LV2_WORKER__INTERFACE: &'static [u8] = b"http://lv2plug.in/ns/ext/worker#interface\0";
pub static LV2_WORKER__SCHEDULE : &'static [u8] = b"http://lv2plug.in/ns/ext/worker#schedule\0";

/**
   Status code for worker functions.
*/
pub enum LV2WorkerStatus {
    LV2WorkerSuccess = 0,
    LV2WorkerErrUnknown = 1,
    LV2WorkerErrNoSpace = 2
}


/** Opaque handle for LV2_Worker_Interface::work(). */
pub type LV2WorkerRespondHandle = *const c_void;
...
*/
pub type LV2WorkerRespondFunction = extern fn(LV2WorkerRespondHandle, u32, *const c_void) -> LV2WorkerStatus;
...
pub struct LV2WorkerInterface {
	...
	*/
    work: extern fn(LV2Handle, LV2WorkerRespondFunction, LV2WorkerRespondHandle, u32, *const c_void) -> LV2WorkerStatus,
	...
	*/
    work_response: extern fn(LV2Handle, u32, *const c_void) -> LV2WorkerStatus,
	...
	*/
    end_run: extern fn(LV2Handle) -> LV2WorkerStatus
}


/** Opaque handle for LV2_Worker_Schedule. */
pub type LV2WorkerScheduleHandle = *const c_void;
...
pub struct LV2WorkerSchedule {
    /**
	   Opaque host data.
	*/
    handle: LV2WorkerScheduleHandle,
	...
	*/
    schedule_work: extern fn(LV2WorkerScheduleHandle, u32, *const c_void) -> LV2WorkerStatus
}